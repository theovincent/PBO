...
then
    ...
    submission_evaluate_dqn=$(sbatch -J A_evaluate_dqn --dependency=afterok:$submission_id_train_dqn --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=9 --mem-per-cpu=5000Mc --time=15:00 --output=out/acrobot/$EXPERIMENT_NAME/evaluate_dqn_%a.out --error=error/acrobot/$EXPERIMENT_NAME/evaluate_dqn_%a.out -p amd,amd2 launch_job/acrobot/evaluate_dqn.sh -e $EXPERIMENT_NAME -b $MAX_BELLMAN_ITERATION)
fi 
...
then
    ...
    submission_evaluate_pbo_linear=$(sbatch -J A_evaluate_pbo_linear --dependency=afterok:$submission_id_train_pbo_linear --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=9 --mem-per-cpu=5000Mc --time=15:00 --output=out/acrobot/$EXPERIMENT_NAME/evaluate_pbo_linear_%a.out --error=error/acrobot/$EXPERIMENT_NAME/evaluate_pbo_linear_%a.out -p amd,amd2 launch_job/acrobot/evaluate_pbo_linear.sh -e $EXPERIMENT_NAME -b $MAX_BELLMAN_ITERATION -a linear)
fi
...
then
    ...
    submission_evaluate_pbo_deep=$(sbatch -J A_evaluate_pbo_deep --dependency=afterok:$submission_id_train_pbo_deep --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=9 --mem-per-cpu=5000Mc --time=15:00 --output=out/acrobot/$EXPERIMENT_NAME/evaluate_pbo_deep_%a.out --error=error/acrobot/$EXPERIMENT_NAME/evaluate_pbo_deep_%a.out -p amd,amd2 launch_job/acrobot/evaluate_pbo_deep.sh -e $EXPERIMENT_NAME -b $MAX_BELLMAN_ITERATION -a deep $CONV)
fi
...
then
    ...
    submission_evaluate_idqn=$(sbatch -J A_evaluate_idqn --dependency=afterok:$submission_id_train_idqn --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=9 --mem-per-cpu=5000Mc --time=15:00 --output=out/acrobot/$EXPERIMENT_NAME/evaluate_idqn_%a.out --error=error/acrobot/$EXPERIMENT_NAME/evaluate_idqn_%a.out -p amd,amd2 launch_job/acrobot/evaluate_idqn.sh -e $EXPERIMENT_NAME -b $MAX_BELLMAN_ITERATION)
fi 