...
then
    ...
    submission_train_dqn=$(sbatch -J A_train_dqn_$MAX_BELLMAN_ITERATION --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=3 --mem-per-cpu=750Mc --time=0:50:00 --output=out/acrobot/$EXPERIMENT_NAME/$MAX_BELLMAN_ITERATION\_train_dqn_%a.out --error=error/acrobot/$EXPERIMENT_NAME/$MAX_BELLMAN_ITERATION\_train_dqn_%a.out -p amd,amd2 launch_job/acrobot/train_dqn.sh -e $EXPERIMENT_NAME -b $MAX_BELLMAN_ITERATION)
...
then
    ...
    submission_train_pbo_linear=$(sbatch -J A_train_pbo_linear_$MAX_BELLMAN_ITERATION --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=3 --mem-per-cpu=750Mc --time=3:30:00 --output=out/acrobot/$EXPERIMENT_NAME/$MAX_BELLMAN_ITERATION\_train_pbo_linear_%a.out --error=error/acrobot/$EXPERIMENT_NAME/$MAX_BELLMAN_ITERATION\_train_pbo_linear_%a.out -p amd,amd2 launch_job/acrobot/train_pbo_linear.sh -e $EXPERIMENT_NAME -b $MAX_BELLMAN_ITERATION -a linear)
...
then
    ...
    submission_train_pbo_deep=$(sbatch -J A_train_pbo_deep_$MAX_BELLMAN_ITERATION --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=5 --mem-per-cpu=1200Mc --time=19:30:00 --output=out/acrobot/$EXPERIMENT_NAME/$MAX_BELLMAN_ITERATION\_train_pbo_deep_%a.out --error=error/acrobot/$EXPERIMENT_NAME/$MAX_BELLMAN_ITERATION\_train_pbo_deep_%a.out -p amd,amd2 launch_job/acrobot/train_pbo_deep.sh -e $EXPERIMENT_NAME -b $MAX_BELLMAN_ITERATION -a deep $CONV)
...
then
    ...
    submission_train_idqn=$(sbatch -J A_train_idqn_$MAX_BELLMAN_ITERATION --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=3 --mem-per-cpu=1200Mc --time=9:30:00 --output=out/acrobot/$EXPERIMENT_NAME/$MAX_BELLMAN_ITERATION\_train_idqn_%a.out --error=error/acrobot/$EXPERIMENT_NAME/$MAX_BELLMAN_ITERATION\_train_idqn_%a.out -p amd,amd2 launch_job/acrobot/train_idqn.sh -e $EXPERIMENT_NAME -b $MAX_BELLMAN_ITERATION)
