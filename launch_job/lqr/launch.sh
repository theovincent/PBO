...
submission_collect_sample=$(sbatch -J c_collect_sample --cpus-per-task=3 --mem-per-cpu=100Mc --time=5:00 --output=out/lqr/$EXPERIMENT_NAME/collect_sample.out --error=error/lqr/$EXPERIMENT_NAME/collect_sample.out -p amd,amd2 launch_job/lqr/collect_sample.sh -e $EXPERIMENT_NAME -b 0)
...
then
    ...
    submission_train_fqi=$(sbatch -J c_train_fqi --dependency=afterok:$submission_id_collect_sample --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=3 --mem-per-cpu=200Mc --time=30:00 --output=out/lqr/$EXPERIMENT_NAME/train_fqi_%a.out --error=error/lqr/$EXPERIMENT_NAME/train_fqi_%a.out -p amd,amd2 launch_job/lqr/train_fqi.sh -e $EXPERIMENT_NAME -b $MAX_BELLMAN_ITERATION)
...
then
    ...
    submission_train_lspi=$(sbatch -J c_train_lspi --dependency=afterok:$submission_id_collect_sample --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=3 --mem-per-cpu=200Mc --time=30:00 --output=out/lqr/$EXPERIMENT_NAME/train_lspi.out --error=error/lqr/$EXPERIMENT_NAME/train_lspi.out -p amd,amd2 launch_job/lqr/train_lspi.sh -e $EXPERIMENT_NAME -b $MAX_BELLMAN_ITERATION)
fi
...
then
    ...
    submission_train_pbo_linear=$(sbatch -J c_train_pbo_linear --dependency=afterok:$submission_id_collect_sample --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=3 --mem-per-cpu=200Mc --time=30:00 --output=out/lqr/$EXPERIMENT_NAME/train_pbo_linear_%a.out --error=error/lqr/$EXPERIMENT_NAME/train_pbo_linear_%a.out -p amd,amd2 launch_job/lqr/train_pbo_linear.sh -e $EXPERIMENT_NAME -b $MAX_BELLMAN_ITERATION -a linear)
...
then
    ...
    submission_train_pbo_custom_linear=$(sbatch -J c_train_pbo_custom_linear --dependency=afterok:$submission_id_collect_sample --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=3 --mem-per-cpu=200Mc --time=30:00 --output=out/lqr/$EXPERIMENT_NAME/train_pbo_custom_linear_%a.out --error=error/lqr/$EXPERIMENT_NAME/train_pbo_custom_linear_%a.out -p amd,amd2 launch_job/lqr/train_pbo_custom_linear.sh -e $EXPERIMENT_NAME -b $MAX_BELLMAN_ITERATION -a custom_linear)
