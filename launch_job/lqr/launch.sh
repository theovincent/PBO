...
submission_collect_sample=$(sbatch -J c_collect_sample --cpus-per-task=3 --mem-per-cpu=500Mc --time=5:00 --output=out/lqr/$EXPERIMENT_NAME/collect_sample.out --error=error/lqr/$EXPERIMENT_NAME/collect_sample.out -p amd,amd2 launch_job/lqr/collect_sample.sh -e $EXPERIMENT_NAME -b 0)
...
then
    ...
    submission_train_fqi=$(sbatch -J c_train_fqi --dependency=afterok:$submission_id_collect_sample --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=3 --mem-per-cpu=750Mc --time=30:00 --output=out/lqr/$EXPERIMENT_NAME/train_fqi_%a.out --error=error/lqr/$EXPERIMENT_NAME/train_fqi_%a.out -p amd,amd2 launch_job/lqr/train_fqi.sh -e $EXPERIMENT_NAME -b $MAX_BELLMAN_ITERATION)
    ...
    submission_evaluate_fqi=$(sbatch -J c_evaluate_fqi --dependency=afterok:$submission_id_train_fqi,$submission_id_collect_sample --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=3 --mem-per-cpu=200Mc --time=10:00 --output=out/lqr/$EXPERIMENT_NAME/evaluate_fqi_%a.out --error=error/lqr/$EXPERIMENT_NAME/evaluate_fqi_%a.out -p amd,amd2 launch_job/lqr/evaluate_fqi.sh -e $EXPERIMENT_NAME -b $MAX_BELLMAN_ITERATION)
fi 
...
then
    ...
    submission_train_lspi=$(sbatch -J c_train_lspi --dependency=afterok:$submission_id_collect_sample --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=3 --mem-per-cpu=750Mc --time=30:00 --output=out/lqr/$EXPERIMENT_NAME/train_lspi.out --error=error/lqr/$EXPERIMENT_NAME/train_lspi.out -p amd,amd2 launch_job/lqr/train_lspi.sh -e $EXPERIMENT_NAME -b $MAX_BELLMAN_ITERATION)
fi
...
then
    ...
    submission_train_pbo_linear=$(sbatch -J c_train_pbo_linear --dependency=afterok:$submission_id_collect_sample --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=3 --mem-per-cpu=750Mc --time=30:00 --output=out/lqr/$EXPERIMENT_NAME/train_pbo_linear_%a.out --error=error/lqr/$EXPERIMENT_NAME/train_pbo_linear_%a.out -p amd,amd2 launch_job/lqr/train_pbo_linear.sh -e $EXPERIMENT_NAME -b $MAX_BELLMAN_ITERATION -a linear)
...
then
    ...
    submission_evaluate_pbo_deep=$(sbatch -J c_evaluate_pbo_deep --dependency=afterok:$submission_id_train_pbo_deep,$submission_id_collect_sample --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=3 --mem-per-cpu=600Mc --time=10:00 --output=out/lqr/$EXPERIMENT_NAME/evaluate_pbo_deep_%a.out --error=error/lqr/$EXPERIMENT_NAME/evaluate_pbo_deep_%a.out -p amd,amd2 launch_job/lqr/evaluate_pbo_deep.sh -e $EXPERIMENT_NAME -b $MAX_BELLMAN_ITERATION -a deep)
fi
...
then
    ...
    submission_train_pbo_custom_linear=$(sbatch -J c_train_pbo_custom_linear --dependency=afterok:$submission_id_collect_sample --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=3 --mem-per-cpu=750Mc --time=30:00 --output=out/lqr/$EXPERIMENT_NAME/train_pbo_custom_linear_%a.out --error=error/lqr/$EXPERIMENT_NAME/train_pbo_custom_linear_%a.out -p amd,amd2 launch_job/lqr/train_pbo_custom_linear.sh -e $EXPERIMENT_NAME -b $MAX_BELLMAN_ITERATION -a custom_linear)
    ...
    submission_evaluate_pbo_custom_linear=$(sbatch -J c_evaluate_pbo_custom_linear --dependency=afterok:$submission_id_train_pbo_custom_linear,$submission_id_collect_sample --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=3 --mem-per-cpu=600Mc --time=10:00 --output=out/lqr/$EXPERIMENT_NAME/evaluate_pbo_custom_linear_%a.out --error=error/lqr/$EXPERIMENT_NAME/evaluate_pbo_custom_linear_%a.out -p amd,amd2 launch_job/lqr/evaluate_pbo_custom_linear.sh -e $EXPERIMENT_NAME -b $MAX_BELLMAN_ITERATION -a custom_linear)
fi
...
then
    ...
    submission_train_pbo_optimal=$(sbatch -J c_evaluate_pbo_optimal --dependency=afterok:$submission_id_collect_sample --cpus-per-task=3 --mem-per-cpu=600Mc --time=10:00 --output=out/lqr/$EXPERIMENT_NAME/evaluate_pbo_optimal.out --error=error/lqr/$EXPERIMENT_NAME/evaluate_pbo_optimal.out -p amd,amd2 launch_job/lqr/evaluate_pbo_optimal.sh -e $EXPERIMENT_NAME -b $MAX_BELLMAN_ITERATION)
fi