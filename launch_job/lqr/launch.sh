...
submission_collect_sample=$(sbatch -J l_collect_sample --cpus-per-task=3 --mem-per-cpu=100Mc --time=5:00 --output=out/lqr/$EXPERIMENT_NAME/collect_sample.out --error=error/lqr/$EXPERIMENT_NAME/collect_sample.out -p amd,amd2 launch_job/lqr/collect_sample.sh -e $EXPERIMENT_NAME -b 0)
...
then
    ...
    submission_train_pbo_optimal=$(sbatch -J l_evaluate_pbo_optimal --dependency=afterok:$submission_id_collect_sample --cpus-per-task=3 --mem-per-cpu=600Mc --time=10:00 --output=out/lqr/$EXPERIMENT_NAME/evaluate_pbo_optimal.out --error=error/lqr/$EXPERIMENT_NAME/evaluate_pbo_optimal.out -p amd,amd2 launch_job/lqr/evaluate_pbo_optimal.sh -e $EXPERIMENT_NAME -b $MAX_BELLMAN_ITERATION)
fi