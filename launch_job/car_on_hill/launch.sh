...
submission_collect_sample=$(sbatch -J collect_sample --cpus-per-task=1 --mem-per-cpu=250Mc --time=50:00 --output=out/car_on_hill/$EXPERIMENT_NAME/collect_sample.out --error=error/car_on_hill/$EXPERIMENT_NAME/collect_sample.out -p amd,amd2,rtx,rtx2 launch_job/car_on_hill/collect_sample.sh -e $EXPERIMENT_NAME -b 0 $COUNT_SAMPLES)
...
then
    ...
    submission_train_fqi=$(sbatch -J train_fqi --dependency=afterok:$submission_id_collect_sample --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=1 --mem-per-cpu=750Mc --time=1:30:00 --output=out/car_on_hill/$EXPERIMENT_NAME/train_fqi_%a.out --error=error/car_on_hill/$EXPERIMENT_NAME/train_fqi_%a.out -p amd,amd2,rtx,rtx2 launch_job/car_on_hill/train_fqi.sh -e $EXPERIMENT_NAME -b $MAX_BELLMAN_ITERATION)
...
then
    ...
    submission_train_pbo_linear=$(sbatch -J train_pbo_linear --dependency=afterok:$submission_id_collect_sample --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=1 --mem-per-cpu=750Mc --time=3:30:00 --output=out/car_on_hill/$EXPERIMENT_NAME/train_pbo_linear_%a.out --error=error/car_on_hill/$EXPERIMENT_NAME/train_pbo_linear_%a.out -p amd,amd2,rtx,rtx2 launch_job/car_on_hill/train_pbo_linear.sh -e $EXPERIMENT_NAME -b $MAX_BELLMAN_ITERATION -a linear)
...
then
    ...
    submission_train_ifqi=$(sbatch -J train_ifqi --dependency=afterok:$submission_id_collect_sample --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=1 --mem-per-cpu=750Mc --time=3:30:00 --output=out/car_on_hill/$EXPERIMENT_NAME/train_ifqi_%a.out --error=error/car_on_hill/$EXPERIMENT_NAME/train_ifqi_%a.out -p amd,amd2,rtx,rtx2 launch_job/car_on_hill/train_ifqi.sh -e $EXPERIMENT_NAME -b $MAX_BELLMAN_ITERATION)
