...
submission_collect_sample=$(sbatch -J C_collect_sample --cpus-per-task=3 --mem-per-cpu=500Mc --time=50:00 --output=out/car_on_hill/$EXPERIMENT_NAME/collect_sample.out --error=error/car_on_hill/$EXPERIMENT_NAME/collect_sample.out -p amd,amd2,rtx,rtx2 launch_job/car_on_hill/collect_sample.sh -e $EXPERIMENT_NAME -b 0 $COUNT_SAMPLES)
...
then
    ...
    submission_train_fqi=$(sbatch -J C_train_fqi --dependency=afterok:$submission_id_collect_sample --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=3 --mem-per-cpu=750Mc --time=1:30:00 --output=out/car_on_hill/$EXPERIMENT_NAME/train_fqi_%a.out --error=error/car_on_hill/$EXPERIMENT_NAME/train_fqi_%a.out -p amd,amd2,rtx,rtx2 launch_job/car_on_hill/train_fqi.sh -e $EXPERIMENT_NAME -b $MAX_BELLMAN_ITERATION)
    ...
    submission_evaluate_fqi=$(sbatch -J C_evaluate_fqi --dependency=afterok:$submission_id_train_fqi,$submission_id_collect_sample --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=9 --mem-per-cpu=100Mc --time=10:00 --output=out/car_on_hill/$EXPERIMENT_NAME/evaluate_fqi_%a.out --error=error/car_on_hill/$EXPERIMENT_NAME/evaluate_fqi_%a.out -p amd,amd2,rtx,rtx2 launch_job/car_on_hill/evaluate_fqi.sh -e $EXPERIMENT_NAME -b $MAX_BELLMAN_ITERATION)
fi 
...
then
    ...
    submission_train_pbo_linear=$(sbatch -J C_train_pbo_linear --dependency=afterok:$submission_id_collect_sample --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=3 --mem-per-cpu=750Mc --time=3:30:00 --output=out/car_on_hill/$EXPERIMENT_NAME/train_pbo_linear_%a.out --error=error/car_on_hill/$EXPERIMENT_NAME/train_pbo_linear_%a.out -p amd,amd2,rtx,rtx2 launch_job/car_on_hill/train_pbo_linear.sh -e $EXPERIMENT_NAME -b $MAX_BELLMAN_ITERATION -a linear)
    ...
    submission_evaluate_pbo_linear=$(sbatch -J C_evaluate_pbo_linear --dependency=afterok:$submission_id_train_pbo_linear,$submission_id_collect_sample --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=9 --mem-per-cpu=300Mc --time=10:00 --output=out/car_on_hill/$EXPERIMENT_NAME/evaluate_pbo_linear_%a.out --error=error/car_on_hill/$EXPERIMENT_NAME/evaluate_pbo_linear_%a.out -p amd,amd2,rtx,rtx2 launch_job/car_on_hill/evaluate_pbo_linear.sh -e $EXPERIMENT_NAME -b $MAX_BELLMAN_ITERATION -a linear)
fi
...
then
    ...
    submission_evaluate_pbo_deep=$(sbatch -J C_evaluate_pbo_deep --dependency=afterok:$submission_id_train_pbo_deep,$submission_id_collect_sample --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=9 --mem-per-cpu=600Mc --time=10:00 --output=out/car_on_hill/$EXPERIMENT_NAME/evaluate_pbo_deep_%a.out --error=error/car_on_hill/$EXPERIMENT_NAME/evaluate_pbo_deep_%a.out -p amd,amd2,rtx,rtx2 launch_job/car_on_hill/evaluate_pbo_deep.sh -e $EXPERIMENT_NAME -b $MAX_BELLMAN_ITERATION -a deep $CONV)
fi
...
then
    ...
    submission_train_ifqi=$(sbatch -J C_train_ifqi --dependency=afterok:$submission_id_collect_sample --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=3 --mem-per-cpu=750Mc --time=3:30:00 --output=out/car_on_hill/$EXPERIMENT_NAME/train_ifqi_%a.out --error=error/car_on_hill/$EXPERIMENT_NAME/train_ifqi_%a.out -p amd,amd2,rtx,rtx2 launch_job/car_on_hill/train_ifqi.sh -e $EXPERIMENT_NAME -b $MAX_BELLMAN_ITERATION)
    ...
    submission_evaluate_ifqi=$(sbatch -J C_evaluate_ifqi --dependency=afterok:$submission_id_train_ifqi,$submission_id_collect_sample --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=9 --mem-per-cpu=150Mc --time=10:00 --output=out/car_on_hill/$EXPERIMENT_NAME/evaluate_ifqi_%a.out --error=error/car_on_hill/$EXPERIMENT_NAME/evaluate_ifqi_%a.out -p amd,amd2,rtx,rtx2 launch_job/car_on_hill/evaluate_ifqi.sh -e $EXPERIMENT_NAME -b $MAX_BELLMAN_ITERATION)
fi