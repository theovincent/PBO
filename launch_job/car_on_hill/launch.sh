...
do  
    ...
    submission_train_fqi=$(sbatch -J train_fqi --mem-per-cpu=8Gc --time=10:00 --output=out/train_fqi_$seed.out launch_job/car_on_hill/train_fqi.sh -s $seed -b $MAX_BELLMAN_ITERATION)
    ...
    submission_evaluate_pbo_linear=$(sbatch -J evaluate_pbo_linear --dependency=afterok:$submission_id_train_pbo_linear --mem-per-cpu=4Gc --time=10:00 --output=out/evaluate_pbo_linear_$seed.out launch_job/car_on_hill/evaluate_pbo_linear.sh -s $seed -b $MAX_BELLMAN_ITERATION -a linear)


    # PBO deep
    echo "launch train pbo deep for seed" $seed 
    submission_train_pbo_deep=$(sbatch -J train_pbo_deep --mem-per-cpu=8Gc --time=10:00 --output=out/train_pbo_deep_$seed.out launch_job/car_on_hill/train_pbo_deep.sh -s $seed -b $MAX_BELLMAN_ITERATION -a deep)
    ...
    submission_evaluate_pbo_deep=$(sbatch -J evaluate_pbo_deep --dependency=afterok:$submission_id_train_pbo_deep --mem-per-cpu=4Gc --time=10:00 --output=out/evaluate_pbo_deep_$seed.out launch_job/car_on_hill/evaluate_pbo_deep.sh -s $seed -b $MAX_BELLMAN_ITERATION -a deep)


    # IFQI
    echo "launch train ifqi for seed" $seed 
    submission_train_ifqi=$(sbatch -J train_ifqi --mem-per-cpu=8Gc --time=10:00 --output=out/train_ifqi_$seed.out launch_job/car_on_hill/train_ifqi.sh -s $seed -b $MAX_BELLMAN_ITERATION)
    ...
    submission_evaluate_ifqi=$(sbatch -J evaluate_ifqi --dependency=afterok:$submission_id_train_ifqi --mem-per-cpu=4Gc --time=10:00 --output=out/evaluate_ifqi_$seed.out launch_job/car_on_hill/evaluate_ifqi.sh -s $seed -b $MAX_BELLMAN_ITERATION)
