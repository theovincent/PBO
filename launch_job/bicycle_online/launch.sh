...
then
    ...
    submission_evaluate_dqn=$(sbatch -J Bon_evaluate_dqn --dependency=afterok:$submission_id_train_dqn --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=9 --mem-per-cpu=100Mc --time=15:00 --output=out/bicycle_online/$EXPERIMENT_NAME/evaluate_dqn_%a.out --error=error/bicycle_online/$EXPERIMENT_NAME/evaluate_dqn_%a.out -p amd,amd2 launch_job/bicycle_online/evaluate_dqn.sh -e $EXPERIMENT_NAME -b $MAX_BELLMAN_ITERATION)
fi 
...
then
    ...
    submission_evaluate_pbo_deep=$(sbatch -J Bon_evaluate_pbo_deep --dependency=afterok:$submission_id_train_pbo_deep --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=9 --mem-per-cpu=600Mc --time=15:00 --output=out/bicycle_online/$EXPERIMENT_NAME/evaluate_pbo_deep_%a.out --error=error/bicycle_online/$EXPERIMENT_NAME/evaluate_pbo_deep_%a.out -p amd,amd2 launch_job/bicycle_online/evaluate_pbo_deep.sh -e $EXPERIMENT_NAME -b $MAX_BELLMAN_ITERATION -a deep $CONV)
fi
