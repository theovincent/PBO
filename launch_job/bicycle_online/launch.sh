...
then
    ...
    submission_evaluate_dqn=$(sbatch -J Bon_evaluate_dqn --dependency=afterok:$submission_id_train_dqn --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=5 --mem-per-cpu=700Mc --time=15:00 --output=out/bicycle_online/$EXPERIMENT_NAME/evaluate_dqn_%a.out --error=error/bicycle_online/$EXPERIMENT_NAME/evaluate_dqn_%a.out -p amd,amd2 launch_job/bicycle_online/evaluate_dqn.sh -e $EXPERIMENT_NAME -b $MAX_BELLMAN_ITERATION)
fi 
