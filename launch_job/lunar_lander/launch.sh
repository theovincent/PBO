...
#     submission_train_idqn=$(sbatch -J train_idqn --dependency=afterok:$submission_id_collect_sample --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=1 --mem-per-cpu=750Mc --time=3:30:00 --output=out/$EXPERIMENT_NAME/train_idqn_%a.out --error=error/$EXPERIMENT_NAME/train_idqn_%a.out -p amd,amd2,rtx,rtx2 launch_job/lunar_lander/train_idqn.sh -e $EXPERIMENT_NAME -b $MAX_BELLMAN_ITERATION)
...
#     submission_evaluate_idqn=$(sbatch -J evaluate_idqn --dependency=afterok:$submission_id_train_idqn,$submission_id_collect_sample --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=9 --mem-per-cpu=150Mc --time=10:00 --output=out/$EXPERIMENT_NAME/evaluate_idqn_%a.out --error=error/$EXPERIMENT_NAME/evaluate_idqn_%a.out -p amd,amd2,rtx,rtx2 launch_job/lunar_lander/evaluate_idqn.sh -e $EXPERIMENT_NAME -b $MAX_BELLMAN_ITERATION)
# fi