...
then
    ...
    submission_train_dqn=$(sbatch -J L_train_dqn --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=3 --mem-per-cpu=750Mc --time=3:30:00 --output=out/$EXPERIMENT_NAME/train_dqn_%a.out --error=error/$EXPERIMENT_NAME/train_dqn_%a.out -p amd,amd2 launch_job/lunar_lander/train_dqn.sh -e $EXPERIMENT_NAME -b $MAX_BELLMAN_ITERATION)
...
then
    ...
    submission_train_pbo_linear=$(sbatch -J L_train_pbo_linear --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=3 --mem-per-cpu=750Mc --time=3:30:00 --output=out/$EXPERIMENT_NAME/train_pbo_linear_%a.out --error=error/$EXPERIMENT_NAME/train_pbo_linear_%a.out -p amd,amd2 launch_job/lunar_lander/train_pbo_linear.sh -e $EXPERIMENT_NAME -b $MAX_BELLMAN_ITERATION -a linear)
...
then
    ...
    submission_train_idqn=$(sbatch -J L_train_idqn --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=3 --mem-per-cpu=1200Mc --time=9:30:00 --output=out/$EXPERIMENT_NAME/train_idqn_%a.out --error=error/$EXPERIMENT_NAME/train_idqn_%a.out -p amd,amd2 launch_job/lunar_lander/train_idqn.sh -e $EXPERIMENT_NAME -b $MAX_BELLMAN_ITERATION)
    ...
    submission_evaluate_idqn=$(sbatch -J L_evaluate_idqn --dependency=afterok:$submission_id_train_idqn --array=$FIRST_SEED-$LAST_SEED --cpus-per-task=9 --mem-per-cpu=4000Mc --time=15:00 --output=out/$EXPERIMENT_NAME/evaluate_idqn_%a.out --error=error/$EXPERIMENT_NAME/evaluate_idqn_%a.out -p amd,amd2 launch_job/lunar_lander/evaluate_idqn.sh -e $EXPERIMENT_NAME -b $MAX_BELLMAN_ITERATION)
fi 