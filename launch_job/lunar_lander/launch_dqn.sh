...
submission_train_dqn_1=$(sbatch -J L_$EXPERIMENT_NAME --array=$FIRST_SEED-$LAST_SEED --ntasks=4 --mem-per-cpu=250M --time=01:00:00 --output=out/lunar_lander/$EXPERIMENT_NAME/train_dqn_%a.out launch_job/lunar_lander/train_dqn.sh -e $EXPERIMENT_NAME)