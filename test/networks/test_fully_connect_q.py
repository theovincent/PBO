...
class TestTableQ(unittest.TestCase):
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.actions_on_max = jnp.array(np.random.randint(-10, 10, size=(np.random.randint(1, 30), self.action_dim)))
    ...
    def test_max_value(self) -> None:
        ...
        for batch_size in range(1, 21):
            ...
            states = np.random.random(size=(batch_size, self.state_dim))

            max_values = np.zeros(batch_size)
            for idx_state, state in enumerate(states):
                state_repeat = jnp.repeat(state.reshape((1, self.state_dim)), self.actions_on_max.shape[0], axis=0)
                max_values[idx_state] = q(params, state_repeat, self.actions_on_max).max()
    ...
    def test_compute_target(self) -> None:
        ...
        for batch_size in range(1, 21):
            ...
            next_states = np.random.random(size=(batch_size, self.state_dim))
            samples = {
